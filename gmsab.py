...
def save_s3(art_data):
    ...
    import subprocess
    s3cmd = subprocess.run(['s3cmd', 'put', art_data['filename'], 's3://gmsab/', '--content-type', 'image/jpeg'])
...
def save_db(art_data):
    ...
    import requests
    import json
    url = open('store.url').readline()  # API url is provided via a local token
    payload = dict()
